#changes to the orginal authenticate.py file from streamlit, for register_user(), login()
...
class Authenticate:
    ...
    def login(self, form_name: str, location: str='main') -> tuple:
        ...
        #     user_now = db.get_user(st.session_state['username'])
        #     st.session_state['name'] == user_now['first_name']
    ...
    def logout(self, button_name: str, location: str='main'):
        ...
        if location == 'main':
            if st.button(button_name):
                ...
                st.session_state['prevUser'] = st.session_state['username']
    ...
    def register_user(self, form_name: str, location: str='main', preauthorization=True) -> bool:
        ...
        new_name = register_user_form.text_input('First name')
        new_lastName = register_user_form.text_input('Last name')
        new_gender = register_user_form.selectbox('Gender',('Male','Female','Other'))
        new_age = register_user_form.date_input('Age')
        new_location = register_user_form.text_input('Location')
        new_password = register_user_form.text_input('Password', type='password')
        new_password_repeat = register_user_form.text_input('Repeat password', type='password')
        
        #user preferences for signup
        diff_shows = df_shows['title'].unique()
        top10_shows = register_user_form.multiselect("Select your top 10 programs",set(diff_shows),max_selections=10)
        ...
        genre = register_user_form.multiselect("Genre",set(genres), max_selections=(len(genres)-1))
        
        #title type
        title_types = df_shows['title_type'].unique()
        title_type = register_user_form.multiselect("Title type", title_types, max_selections=(len(title_types)-1))
        
        ratings = df_shows['rating'].unique()
        ratings = ["No preference" if x is np.nan else x for x in ratings]
        rating = register_user_form.multiselect("Rating", set(ratings), max_selections=3)
        ...
        pref_date = register_user_form.slider("Year of release",min_date,max_date,(2018,2020))
        ...
        if register_user_form.form_submit_button('Register'):
            if len(new_email) and len(new_username) and len(new_name) and len(new_password) > 0:
                if new_username not in self.credentials['usernames']:
                    if new_password == new_password_repeat:
                        new_hashed_password = Hasher([str(new_password)]).generate()
                        if preauthorization:
                            if new_email in self.preauthorized['emails']:
                                ...
                                db.insert_user(new_username, new_name, new_lastName, new_email, new_gender, str(new_age), new_location, new_hashed_password, top10_shows, genre, rating, pref_date,title_type)
                                return True
                            else:
